...
class EmiActivity : BaseActivity(), IBenefitTable, View.OnClickListener {
    ...
    private fun print() {
        ...
        GlobalScope.launch {
            ...
            try {
                ...
                PrintUtil(this@EmiActivity).printTenure(this@EmiActivity,idmList as ArrayList<IssuerDataModel>,mAmount.toFloat())
                /*  else{
    ...
    private fun checkReversal(transactionISOByteArray: IsoDataWriter, cardProcessedDataModal: CardProcessedDataModal, data: HashMap<String, String>) {
        runOnUiThread { showProgress(getString(R.string.sale_data_sync)) }
        if (TextUtils.isEmpty(AppPreference.getString(AppPreference.GENERIC_REVERSAL_KEY))) {
            SyncEmiEnquiryTransactionToHost(transactionISOByteArray, cardProcessedDataModal) { syncStatus, responseCode, transactionMsg, printExtraData ->
                hideProgress()
    ...
    override fun onEvents(event: VxEvent) {
        when (event) {
            is VxEvent.PayEmi -> {
                val total = event.amount + event.tenure.proccesingFee.toFloat()/100+((event.tenure.processingRate.toFloat()/100) * (event.amount-(event.tenure.emiAmount?.discount ?: 0f)))/100 - (event.tenure.emiAmount?.cashBack
                    ?: 0f) - (event.tenure.emiAmount?.discount ?: 0f) + (event.tenure.emiAmount?.totalInterest ?: 0f)
             /*   val total =
              ...
              //      loanAmt = "%d".format((mAmount * 100).toInt())/*event.tenure.emiAmount?.getTransactionAmt() ?: ""*/
                    var loanamounts1 = "${((event.tenure.emiAmount?.principleAmt ?: 0f)-(event.tenure.emiAmount?.discount ?: 0f)).toFormatString()}"
               ...
               //     netPay = "%d".format((total * 100).toInt())
                    processingFee = ((event.tenure.proccesingFee.toFloat()/100)+((event.tenure.processingRate.toFloat()/100) * ((event.amount-(event.tenure.emiAmount?.discount ?: 0f)))/100)).toFormatString()
...
class EmiPagerAdapter(fm: FragmentManager, var list: List<EmiDetailFragment>, var title: List<String>) : FragmentStatePagerAdapter(fm) {
...
class SchemeAdapter(private var emiList: List<EmiView>, private var amount: Float) :
    ...
    override fun onCreateViewHolder(viewGroup: ViewGroup, i: Int): RecyclerView.ViewHolder {
        ...
        val v = LayoutInflater.from(viewGroup.context).inflate(
            if (i == EmiViewType.SCHEME.ordinal) R.layout.item_scheme_v2 else R.layout.item_emi_detail, viewGroup, false)
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, i: Int) {
        ...
        if (emiList[i].type.ordinal == EmiViewType.SCHEME.ordinal) {
            ...
            val totalPayment = amount + data.proccesingFee.toFloat()/100+((data.processingRate.toFloat()/100) * (amount-(data.emiAmount?.discount ?: 0f)))/100 - (data.emiAmount?.cashBack
                ?: 0f) - (data.emiAmount?.discount ?: 0f) + (data.emiAmount?.totalInterest ?: 0f)
