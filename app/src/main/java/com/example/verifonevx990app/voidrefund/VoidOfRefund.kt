                ...
                if (voidRefundCB) {
                    ...
                    if (voidRefundCB && responseCode == "00") {
                        ...
                        if (VFService.vfPrinter?.status == 0) {
                            VoidRefundSalePrintReceipt().startPrintingVoidRefund(voidRefundBatchData, TransactionType.VOID_REFUND.type, EPrintCopyType.MERCHANT, activity as BaseActivity) { printCB, printCBC ->
                                if (printCB) {
                                    VoidRefundSalePrintReceipt().startPrintingVoidRefund(voidRefundBatchData, TransactionType.VOID_REFUND.type, EPrintCopyType.CUSTOMER, activity as BaseActivity) { printCB, printCBC_ ->
                                        if (printCBC_) {
