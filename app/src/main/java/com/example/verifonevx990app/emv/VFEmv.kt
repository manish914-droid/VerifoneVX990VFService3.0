...
object VFEmv : ISO8583(), OnXmlDataParsed {
    ...
    fun initializeEMVVoid(vfIEMV: IEMV?,
                          vfPinPad: IPinpad?,
        ...
        emvHandler = object : EMVHandler.Stub() {
            ...
            override fun onRequestOnlineProcess(aaResult: Bundle) {
                ...
                if(isNoPin as Boolean){
                    TransactionTypeValues.saleType = TransactionTypeValues.SALETYPE.EMV_POS_ENTRY_NO_PIN
                }

                val result = aaResult.getInt(ConstPBOCHandler.onRequestOnlineProcess.aaResult.KEY_RESULT_int)
                val signature = aaResult.getBoolean(ConstPBOCHandler.onRequestOnlineProcess.aaResult.KEY_SIGNATURE_boolean)
    ...
    private fun createNormalPackage(isoPackageWriter: IsoPackageWriter?, transactionalAmmount: Long, cashBackAmount: Long, xmlFieldModels: HashMap<String, XmlFieldModel>?) {
...
//                    isoPackageWriter.operationType = "MAG STRIPE"
    ...
    private fun createVoidPackage(isoPackageWriter: IsoPackageWriter?): IsoPackageWriter? {
        try {
            ...
            //end of tpdu//
            //processing code
            isoPackageWriter.processingCode = TransactionTypeValues.getProcCode(transactionType)//"920001"
            packageWriterModel!!.processingCode = isoPackageWriter.getProccingCode()
            isoPackageWriter.setFieldValues("3", isoPackageWriter.getProccingCode()!!)//3 byte

            //transation ammount
            isoPackageWriter.transactionalAmmount = (transactionalAmmount + cashBackAmount).toString()//batchFileDataTable!!.totalAmmount
            packageWriterModel!!.transactionalAmmount = isoPackageWriter.transactionalAmmount
            isoPackageWriter.setFieldValues("4", isoPackageWriter.transactionalAmmount!!)//6 byte
            ...
            isoPackageWriter.roc = HexStringConverter.addPreFixer(terminalParameterTable!!.stan, 6)//HexStringConverter.addPreFixer(AppPreference.getNewInstance().getStanNumber(AppPreference.STAN_KEY, context).toString(), 6)//
            CustomToast.printAppLog(isoPackageWriter.roc)
            packageWriterModel!!.roc = isoPackageWriter.roc
            CustomToast.printAppLog(isoPackageWriter.roc)
            isoPackageWriter.setFieldValues("11", isoPackageWriter.roc!!)////"" + AppPreference.getNewInstance().getStanNumber(STAN_KEY, context)
            try {
                val date: Long = Calendar.getInstance().timeInMillis
                ...
                packageWriterModel!!.timeStamp = isoPackageWriter.timeStamp
                isoPackageWriter.setFieldValues("13", isoPackageWriter.date!!)
                isoPackageWriter.posEntryValue = batchFileDataTable!!.posEntryValue//TransactionTypeValues.getPosValue().toString()
                packageWriterModel!!.posEntryValue = isoPackageWriter.posEntryValue
                isoPackageWriter.setFieldValues("22", isoPackageWriter.posEntryValue!!)

                isoPackageWriter.nii = HexStringConverter.addPreFixer(batchFileDataTable!!.nii, 4)//"0091"
                packageWriterModel!!.nii = isoPackageWriter.nii
                isoPackageWriter.setFieldValues("24", isoPackageWriter.nii!!)
                //tid
                isoPackageWriter.tid = HexStringConverter.addPreFixer(batchFileDataTable!!.tid, 8)//"10600003"//
                packageWriterModel!!.tid = isoPackageWriter.tid
                isoPackageWriter.setFieldValues("41", isoPackageWriter.tid)//20200002

                isoPackageWriter.mid = HexStringConverter.addPreFixer(batchFileDataTable!!.mid, 15)//"000000010610002"//630000000000010
                packageWriterModel!!.mid = isoPackageWriter.mid
                isoPackageWriter.setFieldValues("42", isoPackageWriter.mid!!)
                ...
                if (isoPackageWriter.transactionType == TransactionTypeValues.VOID || isoPackageWriter.transactionType == TransactionTypeValues.REVERSAL || isoPackageWriter.transactionType == TransactionTypeValues.SALE_COMPLETION || isoPackageWriter.transactionType == TransactionTypeValues.SALE_WITH_TIP || isoPackageWriter.transactionType == TransactionTypeValues.TIP_ADJUSTMENT) {
                    ...
                    isoPackageWriter.setFieldValues("56", isoPackageWriter.field56)//need to set here
                    packageWriterModel!!.field56 = isoPackageWriter.field56
                }

                if (batchFileDataTable?.track2Data != null && batchFileDataTable!!.track2Data.isNotEmpty() && HexStringConverter.hexToString(batchFileDataTable!!.track2Data)
                ...
                isoPackageWriter.batchNumber = HexStringConverter.addPreFixer(batchFileDataTable!!.batchNumber, 6)//"000003"
                packageWriterModel!!.batchNumber = isoPackageWriter.batchNumber
                ...
                isoPackageWriter.setFieldValues("61", isoPackageWriter.terminalSerialNumber + isoPackageWriter.bankCode + isoPackageWriter.customerId + isoPackageWriter.walletIssuerId + isoPackageWriter.connectionType + isoPackageWriter.modelName + isoPackageWriter.appName + isoPackageWriter.appVersion + isoPackageWriter.pcNumber)
                isoPackageWriter.invoiceNumber = invoiceWithPadding(batchFileDataTable!!.invoiceNumber)
                packageWriterModel!!.invoiceNumber = isoPackageWriter.invoiceNumber
