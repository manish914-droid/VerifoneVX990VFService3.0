...
class SchemeDataModel() : Parcelable, EmiView(EmiViewType.SCHEME) {
    ...
    private constructor(parcel: Parcel) : this() {
        ...
        tenureDataModel = parcel.readParcelableArray(TenureDataModel::class.java.classLoader) as Array<TenureDataModel>
...
enum class EAccountType(val code: String) { DEFAULT("00"), SAVING("10"), CREDIT("30"), CHEQUE("20"), UNIVERSAL("40") }
...
fun flatEmi(amount: Long, rate: Float, nom: Int): Float = (amount.toFloat() / (1200 * nom)) * (1200 + (rate * nom))
...
fun attachEmiAmountWithValidation(scheme: SchemeDataModel, tenureDataModel: TenureDataModel, amount: Float, benefitGetter: IBenefitTable?) {
 ...
 //           emiScheme ->
    val emiScheme = benefitGetter?.getEmiSchemeTable(scheme.schemeId)
        ...
        val getCalculation: (str: String) -> EBenefitCalculation = {
            benefitSlabTable = benefitGetter.getBenifitTable(emiScheme.emiSchemeId, tenureDataModel.tenure)
            when (it) {
        ...
        if (amount <= emiScheme.maxValue.toDouble() / 100 && amount >= emiScheme.minValue.toDouble() / 100) {
            ...
            if(currDt.compareTo(emiScheme.startDate) >= 0 && currDt.compareTo(emiScheme.endDate) <= 0) {
                val emiAmount = when (emiScheme.benifitModelId) {
                    "7202" -> {
                        EmiAmounts(amount, roi, tenureDataModel.tenure.toInt(), EBenefitModel.CASH_BACK, emiScheme.maxAmount.toFloat()).apply {
                            // Cash back
                            addCashBack(getCalculation(emiScheme.benifitCalculationRefundId), getCalcValue(emiScheme.benifitCalculationRefundId), tenureDataModel)
                        }
                    }
                    "7203" -> {
                        EmiAmounts(amount, roi, tenureDataModel.tenure.toInt(), EBenefitModel.DISCOUNT, emiScheme.maxAmount.toFloat()).apply {
                            // Discount
                            addDiscount(getCalculation(emiScheme.benifitCalculationRefundId), getCalcValue(emiScheme.benifitCalculationRefundId), tenureDataModel,emiScheme)
                        }
                    }
                    //CIB 7201
                    else -> EmiAmounts(
...
enum class EBenefitCalculation(val type: Byte) { TOTAL_CALCULATED_INTEREST(1), FIXED_VALUE(2), PERCENTAGE_VALUE(3) }

enum class EBenefitModel(val type: Byte) { CUSTOMER_INTEREST_BEAR(1), CASH_BACK(2), DISCOUNT(3) }

class EmiAmounts( val principleAmt: Float, val roi: Float = 0f, noOfMonth: Int = 0,
                 val benefitModel: EBenefitModel = EBenefitModel.CUSTOMER_INTEREST_BEAR, private val maxAmount: Float = 0f) {
    var monthlyEmi = 0f
    ...
    fun addDiscount(type: EBenefitCalculation, value: Float = 0f, tenureDataModel: TenureDataModel, emiScheme: EmiSchemeTable) {
    ...
    private fun checkWithMaxAmtDiscount(amt: Float, tenureDataModel: TenureDataModel): Float = if (amt <= maxAmount/100) {
        amt
    ...
    private fun recaclulateEmi(value: Float, tenureDataModel: TenureDataModel) : Float{
        monthlyEmi = reducingBalanceEmi(principleAmt - value, roi, tenureDataModel.tenure.toInt()).round()
    ...
    fun addCashBack(type: EBenefitCalculation, value: Float = 0f, tenureDataModel: TenureDataModel) {
