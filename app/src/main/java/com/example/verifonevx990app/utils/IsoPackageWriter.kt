...
open class IsoPackageWriter() : Serializable {
    ...
    constructor(context: Context?, onXmlDataParsed: OnXmlDataParsed?,pcNo:Int):this(context,onXmlDataParsed){
...
//
    ...
    fun observerToCreatePackage(transactionType: Int, xmlFieldModels: HashMap<String, XmlFieldModel>): Observable<StringBuilder> {
        return Observable.create { emitter ->
            try {
    ...
    //creates the package
    private fun createPackageData(transactionType: Int, xmlFieldModels: HashMap<String, XmlFieldModel>): StringBuilder {
        ...
        for (isoFiledLength in isoPackageWriters) {
            ...
            transactionHandling.fieldNumber(Integer.parseInt(xmlFieldModel!!.id), isoFiledLength, transactionType, true)
        }
    ...
    private fun setHraderANDMTIBITMAPBasedOnTransactionType(stringBuilder: StringBuilder) {
        ...
        stringBuilder.insert(0, byteShiftOperation(HexStringConverter.binaryToString(bitMapCraeter), 8))
        length += 8
