            ...
            if (voidOfflineCB) {
                GlobalScope.launch(Dispatchers.Main) {
                    ...
                    if (VFService.vfPrinter?.status == 0) {
                        VoidOfflineSalePrintReceipt().voidOfflineSalePrint(activity, voidOfflineBatchData, TransactionType.VOID_OFFLINE_SALE.type, EPrintCopyType.MERCHANT) { voidPrinterCB,voidPrinterCBC ->
                            if (voidPrinterCB) {
                                VoidOfflineSalePrintReceipt().voidOfflineSalePrint(activity, voidOfflineBatchData, TransactionType.VOID_OFFLINE_SALE.type, EPrintCopyType.CUSTOMER) { voidPrinterCB, voidPrinterCBC ->
