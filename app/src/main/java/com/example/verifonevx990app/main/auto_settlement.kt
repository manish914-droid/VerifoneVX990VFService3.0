...
suspend fun setAutoSettlement() {
    try {
        ...
        if (tpt != null) {
            val alMag = VerifoneApp.appContext.getSystemService(Context.ALARM_SERVICE) as AlarmManager

            val alarmIntent: PendingIntent
            alarmIntent = Intent(VerifoneApp.appContext, AutoSettleReceiver::class.java).let { intent ->
                intent.action = "com.bonushub.vxPos.force_settle"
                intent.addCategory("android.intent.category.DEFAULT")
                PendingIntent.getBroadcast(VerifoneApp.appContext, 0, intent, PendingIntent.FLAG_UPDATE_CURRENT)
            }
